...
def shortener(string):
    ...
    new_list = []
    for long_word in list_of_words:
        if len(long_word) > 6:
            long_word = long_word[:6]
            long_word += '*'
...
def compare_ends(words):
    """
    Function receives an arra of strings.
    ...
    """
    return len([short_word for short_word in words if len(short_word) > 1 and short_word[0] == short_word[-1]])
